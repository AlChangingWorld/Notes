...
``` c#
Lesson3_Test t = this.GetComponentInChildren<Lesson3_Test>(true); //默认是false 失活子对象object 不会找 true 失火也找

// 得子对象多个
Lesson3_Test[] lts = this.GetComponentsInChildren<Lsson3_Test>(true);
print(lts.Length);

List<Lsson3_Test> list2 = new List<Lsson3_Test>();
this.GetComponentsInChildren<Lesson3>(true, list2);
print(list.Count);
```
...
this.transform.position.x = new Vector(10,10,10);
this.transform.localPosition = Vector3.up*10;

// 只想改一个x
// 1赋值改变
this.transform.position.x = new Vector(10,this.transform.position.y,this.transform.position.z);
...
将所有子对象加入List容器再使用sort()lambad表达式进行排序规则返回-1/1                  
然后在变换子对象的 编号顺序                 
...
**Depth only**      
让深度高的摄像机选择该模式 只渲染这个深度Culling Mask的东西 其他是透明的            
俩个摄像机可以叠加显示              
UI 会使用专门的一个摄像机来管理         
...
3. 如果是一个异形物体，刚体在父对象上，钢铁上
如果你想通过子对象上挂脚本检测碰撞是不行的
必须挂载到这个刚体父对象上才行 
...
- 加力 rigidBody.AddForce rigidBody.AddRelativeForce
- 直接改变刚体速度变量 rigidBody.velocity = Vector3.forward*10;
